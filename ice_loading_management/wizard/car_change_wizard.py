...
class CarChangeWizard(models.TransientModel):
    ...
    current_car_name = fields.Char(related='current_car_id.license_plate', string='Current Car License')
    ...
    new_car_name = fields.Char(related='new_car_id.license_plate', string='New Car License')
    ...
    def _compute_status_warning(self):
        for wizard in self:
            # if wizard.current_state in ['ready_for_loading', 'loaded', 'freezer_loaded', 'plugged', 'in_transit', 'delivered']:
            if wizard.current_state in ['ready_for_loading', 'ice_handled', 'plugged', 'in_transit', 'delivered']:
                wizard.status_warning = _("⚠️ WARNING: Car changes are not allowed in '%s' status!") % dict(
                    wizard.loading_request_id._fields['state'].selection
                )[wizard.current_state]
            else:
                wizard.status_warning = _("✅ Car change is allowed in current status.")
    ...
    def action_change_car(self):
        ...
        # if self.current_state in ['ready_for_loading', 'loaded', 'freezer_loaded', 'plugged', 'in_transit', 'delivered']:
        if self.current_state in ['ready_for_loading', 'ice_handled', 'plugged', 'in_transit', 'delivered']:
            raise UserError(_(
                'Cannot change car: Loading request is in "%s" status. '
                'Car changes are only allowed before the "Ready for Loading" stage.'
            ) % dict(self.loading_request_id._fields['state'].selection)[self.current_state])
        ...
        self.loading_request_id.write({
            ...
            'car_changed_by': self.env.user.id,
        ...
        if previous_car.loading_status == 'in_use':
            previous_car.loading_status = 'available'
        self.new_car_id.loading_status = 'in_use'
        
        # Update maintenance request if exists
        # if self.loading_request_id.car_check_request_id:
        #     self.loading_request_id.car_check_request_id.vehicle_id = self.new_car_id.id
